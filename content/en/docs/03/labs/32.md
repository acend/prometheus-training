...
{{% onlyWhenNot baloise %}}
  * Select the **thanos-querier** data source
  * Add the expression from above into the text field right next to the **Metrics Browser** dropdown
  * Set the panel title to `Memory Used` under **Panel options > Title** (you may need to open 
{{% /onlyWhenNot %}}
{{% onlyWhen baloise %}}
  * Select the **thanos-querier** data source
  * Add the expression from above into the text field right next to the **Metrics Browser** dropdown
  * Set the panel title to `Memory Available` under **Panel options > Title** (you may need to open 
{{% /onlyWhen %}}
the options pane with the **<** button on the right hand side just below the **Apply** button)
...
* On your [Grafana web UI](https://{{% param grafanaUrl %}})
  ...
  * Copy the JSON content and save the file `user-demo/training_dashboard.yaml`:
