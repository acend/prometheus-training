...
You can stop the application scaling down the deployment, by set the `replicas` in the `user-demo/deployment.yaml` to 0. Then commit and push the changes.


* The Prometheus web UI **Alerts** [menu item](https://{{% param alertmanagerUrL %}}/alerts) shows you information about inactive and active alerts.
* As soon as an alert is in state `FIRING` the alert is sent to Alertmanager. You should see the alert in its [web UI](https://{{% param alertmanagerUrL %}}).
