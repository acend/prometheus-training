...
Let's now create a query which selects all metrics belonging to the blackbox exporter target `https://bitbucket.balgroupit.com/status` and display them in the [Thanos Querier UI](http://{{% param replacePlaceholder.thanos %}}).
...
or directly navigate to your [Thanos Querier](http://{{% param replacePlaceholder.thanos %}}/graph?g0.expr=%7Binstance%3D"https%3A%2F%2Fbitbucket.balgroupit.com%2Fstatus"%7D&g0.tab=1)
